...
function Produto({ navigation }) {
    ...
    return (
        <ScrollView>
            <View className="bg-purple-950 h-full w-full">
                <HeaderPersonalizado />
                <View className="w-full flex flex-col items-center">
                    <View className="w-[90%] flex flex-row space-x-4 ">
                        <View className="w-[50%] max-h-48">
                            <GetGameImage game={game} />
                        </View>
                        <View className="w-full space-y-2">
                            <View className="w-full flex flex-row-reverse">
                                <TouchableOpacity className="w-[10%] rounded-md bg-[#D32424]  flex justify-center items-center">
                                    <Image className="w-[22px]" source={require("../../assets/imagens/heart.png")} />
                                </TouchableOpacity>
                            </View>
                            <Text className=" text-white text-base">{game.data.name}</Text>
                            <Text className="text-white text-sm">⭐⭐⭐⭐⭐</Text>
                            <Text className="text-white text-lg">R$ {game.data.price}</Text>
                            <Text className="text-white text-sm">30% de desconto no pix</Text>
                        </View>
                    </View>
                    <View className="w-full flex items-center gap-y-8">
                        ...
                        <View className="w-[90%] flex items-center">
                            <View className="w-full flex flex-row justify-between">
                                <TouchableOpacity onPress={comprar} className="w-full rounded-md bg-[#1AB82A] h-14 ">
